...
namespace University
{
    class Initilization
    {
        ...
        static public Student GetRandomStudent()
        {
            Student person = new Student() { Gender = GetRandomGenderOfStudent() };
            person.Name = GetRandomNameOfStudent(person.Gender);
            person.INN = GetRandomInnOfStudent();
            return person;
        }

        static public Group GetRandomInitilizationGroup()
        {
            Group form = new Group() { CountStudents = GetRandomCountOfGroup(), Name = GetRandomNameOfGroup() };
            form.Students = new Student[form.CountStudents];
           
            for (int i = 0; i < form.Students.Length; i++)
            {

                form.Students[i] = GetRandomStudent();
